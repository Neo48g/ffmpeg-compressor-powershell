...
function Show-Menu {
    ...
    Write-Host "================================================================" -ForegroundColor Cyan; Write-Host ""
    $fc = Get-VideoFilesCount; Write-Host "$(L 'Input folder' 'Папка ввода'): $inputFolder" -ForegroundColor Gray
    Write-Host "$(L 'Video files found' 'Найдено видеофайлов'): " -NoNewline -ForegroundColor Gray; if ($fc -gt 0) { Write-Host "$fc" -ForegroundColor Green } else { Write-Host "$fc" -ForegroundColor Red }
    ...
    Write-Host "----------------------------------------------------------------" -ForegroundColor Cyan
    Write-Host "  1. $(L 'Change input folder' 'Изменить папку ввода')"
    Write-Host "  2. $(L 'Toggle recursive search' 'Переключить рекурсивный поиск')"
    Write-Host "  H. $(L 'Hardware Acceleration' 'Аппаратное ускорение')"
    Write-Host "  D. $(L 'Auto-detect hardware' 'Автоопределение оборудования')"
    Write-Host "  P. $(L 'Presets management' 'Управление пресетами')"
    Write-Host "  Q. $(L 'Change quality value' 'Изменить значение качества') ($crfValue)"
    Write-Host "  V. $(L 'Toggle VMAF' 'Переключить VMAF')"
    Write-Host "  A. $(L 'Toggle Auto CRF' 'Переключить Авто CRF')"
    Write-Host "  T. $(L 'Change VMAF threshold' 'Изменить порог VMAF') ($vmafThreshold)"
    Write-Host "  I. $(L 'Change max iterations' 'Изменить макс. итераций') ($maxIterations)"
    Write-Host "  M. $(L 'Change minimum CRF' 'Изменить мин. CRF') ($minCRF)"
    Write-Host "  L. $(L 'Logs management' 'Управление логами')"
    Write-Host "  G. $(L 'Change language' 'Изменить язык')"
    Write-Host "  W. $(L 'Help & Guide' 'Справка и руководство')" -ForegroundColor Magenta
    Write-Host "  F. $(L 'FFmpeg: Check status, install, or view details' 'FFmpeg: Проверить статус, установить или просмотреть детали') ($(Get-FFmpegVersionDisplay))" -ForegroundColor DarkGreen
    Write-Host "  S. $(L 'Save settings now' 'Сохранить настройки сейчас')"
            ...
            if ($LASTEXITCODE -eq 0 -and (Test-Path $outputFile)) {
                ...
                if ($enableVMAF) {
                    ...
                    if ($vw -and $vh -and $vfs) {
                        ...
                        $vmafArgs = "-i `"$inputFile`" -i `"$outputFile`" -lavfi `"[0:v]scale=$vw`:$vh,fps=$vmafFps[ref];[1:v]scale=$vw`:$vh,fps=$vmafFps[dist];[ref][dist]libvmaf`" -f null -"; $Host.PrivateData.ProgressForegroundColor = "Cyan"
